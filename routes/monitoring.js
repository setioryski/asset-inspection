...
router.get('/monitoring', isAuthenticated, checkRole(['admin']), async (req, res) => {
  try {
    ...
    // 2. Existing Monitoring Query (with CTE)
    const monitoringSql = `
      WITH cte_aset AS (
          SELECT 
    ...
    // 3. Query for total aset per lantai
    const asetSql = `
      SELECT 
          tl.id AS lantai_id,
          tl.nama_lantai,
    ...
    // 4.2. Tipe Aset - Hilang (id_kondisi = 3)
    ...
    // 4.3. Tipe Door - Rusak (id_kondisi = 2)
    ...
    // 4.4. Tipe Door - Hilang (id_kondisi = 3)
    ...
    // 4.5. Tipe HB - Rusak (id_kondisi = 2)
    ...
    // 4.6. Tipe HB - Hilang (id_kondisi = 3)
    ...
    // 5. Render the view and pass all data
    res.render('monitoring', {
      data: monitoringData,
      asetPerLantai: asetPerLantai,
      tipeAsetRusak: tipeAsetRusak || [],
      tipeAsetHilang: tipeAsetHilang || [],
      tipeDoorRusak: tipeDoorRusak || [],
      tipeDoorHilang: tipeDoorHilang || [],
      tipeHbRusak: tipeHbRusak || [],
      tipeHbHilang: tipeHbHilang || [],
      startDate,
      endDate
    });
  } catch (err) {
